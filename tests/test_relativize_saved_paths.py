...
class RelativizeSavedPathsTests(unittest.TestCase):
    ...
    def test_rebases_paths_from_old_checkout_before_relativizing(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            base = Path(temporary)
            old_repository = Path("/old/machine/project")
            new_repository = base / "new_checkout"
            record = new_repository / "scan_output/run_36/manifest.json"
            record.parent.mkdir(parents=True)
            record.write_text(
                json.dumps(
                    {
                        "configuration": str(
                            old_repository / "scripts/path_planning_single/config.yaml"
                        )
                    }
                ),
                encoding="utf-8",
            ...
            count = relativize_json_file(
                ...
                dry_run=False,
                backup=False,
                destination_repository_root=new_repository,
            )
            updated = json.loads(record.read_text(encoding="utf-8"))

            self.assertEqual(count, 1)
            self.assertEqual(
                updated["configuration"], "../../scripts/path_planning_single/config.yaml"
            )
