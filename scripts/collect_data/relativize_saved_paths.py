...
def relativize_value(
    value: Any, repository_root: Path, record_directory: Path
) -> tuple[Any, int]:
    """Recursively convert strings rooted inside the selected repository."""
    if isinstance(value, dict):
        updated: dict[str, Any] = {}
        replacements = 0
        for key, item in value.items():
            updated_item, count = relativize_value(
                item, repository_root, record_directory
            )
    ...
    try:
        ...
        return value, 0
    return serialized_relative_path(normalized, record_directory), 1
...
def relativize_json_file(
    ...
    *,
    ...
    updated, replacements = relativize_value(
        document, repository_root, path.parent
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="absolute path prefix to convert (default: repository root)",
    )
